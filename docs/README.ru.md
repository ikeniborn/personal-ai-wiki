...
В проде также задаются `POSTGRES_PASSWORD`, `PAW_HOST` (публичный DNS),
`ACME_EMAIL` и опциональные параметры бэкапа. См. **prod-чеклист** в
...
```bash
cp .env.example .env          # затем заполните SESSION_SECRET + FERNET_KEY (см. выше)
