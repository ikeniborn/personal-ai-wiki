...
- **Best-effort on the write path:** the ingest/edit/rollback call sites wrap `project_article` in a `session.begin_nested()` SAVEPOINT and catch any error, so a missing-or-broken graph (e.g. AGE enabled on a domain before its first `graph_rebuild`) logs and rolls back only the graph writes while the relational write still commits. Projection never fails a relational operation — the same degradation guarantee retrieval has; the graph re-syncs on the next rebuild.
