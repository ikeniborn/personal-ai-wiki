...
## Overview
Ingest turns raw uploaded bytes into embeddable text. Per-format loaders in `ingest/loaders/*` normalize md/pdf/docx/html down to a single markdown string; then `chunking.py` splits that markdown by `##` sections, breaks each into sentences, and greedily groups them into overlapping chunks sized by `WikiConfig`. The chunks feed embedding for hybrid retrieval. See [[storage#Backends]], [[vector#Embeddings]], [[harness#Ops]].

## Loaders
Each loader (`ingest/loaders/<fmt>.py`) exposes a single `load(data: bytes) -> str` that returns markdown-ish text. `load_source(data, type)` dispatches by source type; ingest pulls the raw bytes from storage first (see [[storage#PostgresStorage]]).
