...
| `POSTGRES_PASSWORD` | **yes** | — | Strong Postgres superuser password; Compose refuses unset/empty values. If it contains `$`, single-quote it in `.env` or escape `$` as `$$`. |
...
`SESSION_SECRET`, `FERNET_KEY`, `DATABASE_URL`, `REDIS_URL`, and `POSTGRES_PASSWORD` have
no usable Compose defaults and will fail validation on startup if absent. All others have
usable defaults for local dev.
...
- [ ] `POSTGRES_PASSWORD` is a strong random value; `$` is single-quoted or escaped as `$$` in `.env`
