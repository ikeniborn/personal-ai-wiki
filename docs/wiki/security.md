...
- `_escape_body` backslash-escapes `[:LABEL]` edge syntax inside the dollar-quoted body so SQLAlchemy's `text()` parser does not mistake the colon for a bind parameter — a correctness guard, not a user-data path (bodies stay fixed literals; named-node syntax `(a:Article)` and map keys `{id: $id}` need no escaping).
