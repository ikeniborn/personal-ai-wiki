...
# SSE comment line: ignored by EventSource clients, keeps idle connections warm
# so a reverse proxy does not drop them before the terminal event arrives.
...
async def sse_events(
    redis: Any, job_repo: JobRepo, job_id: uuid.UUID, *, idle_timeout: float = 30.0
) -> AsyncIterator[str]:
    ...
    # get_message returns None both on a real idle timeout AND on an ignored
    # subscribe-ack; only emit a keep-alive once a full idle_timeout of silence
    # has actually elapsed, so the ack does not produce a spurious frame.
    last = time.monotonic()
    try:
        while True:
